...
class EstadoController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($buscar==''){
            $consultas= Estado::orderBy('id','desc')->paginate(10);
        }else{
            $consultas= Estado::where($criterio,'like','%'.$buscar.'%')->orderBy('id','desc')->paginate(10);
        }
    ...
    public function selectEstado(Request $request){
         if(!$request->ajax()) return redirect('/');
         $consultas=Estado::where('condicion','=','1')
    ...
    {
        if(!$request->ajax()) return redirect('/');
        $registro= Estado::findOrFail($request->id);
        $registro->nombre=$request->nombre;
