...
class MateriaController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($buscar==''){
            $consultas= Materia::orderBy('id','desc')->paginate(10);
        }else{
            $consultas= Materia::where($criterio,'like','%'.$buscar.'%')->orderBy('id','desc')->paginate(10);
        }
    ...
    public function selectMateria(Request $request){
         if(!$request->ajax()) return redirect('/');
         $consultas=Materia::where('condicion','=','1')
    ...
    {
        if(!$request->ajax()) return redirect('/');
        $registro= Materia::findOrFail($request->id);
        $registro->nombre=$request->nombre;
