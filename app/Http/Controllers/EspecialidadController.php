...
class EspecialidadController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($buscar==''){
            $consultas= Especialidad::orderBy('id','desc')->paginate(10);
        }else{
            $consultas= Especialidad::where($criterio,'like','%'.$buscar.'%')->orderBy('id','desc')->paginate(10);
        }
    ...
    public function selectEspecialidad(Request $request){
         if(!$request->ajax()) return redirect('/');
         $consultas=Especialidad::where('condicion','=','1')
    ...
    {
        if(!$request->ajax()) return redirect('/');
        $registro= Especialidad::findOrFail($request->id);
        $registro->nombre=$request->nombre;
